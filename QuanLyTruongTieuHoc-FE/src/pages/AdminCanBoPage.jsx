...
export default function AdminCanBoPage() {
  ...
  const loadData = async () => {
    const res = await axios.get('http://localhost:3000/can-bo')
    setList(res.data)
  }
  ...
  const handleSearch = async (keyword) => {
    ...
    const res = await axios.get(
      `http://localhost:3000/can-bo/search?keyword=${keyword}`
    )
  ...
  const handleSubmit = async () => {
    if (editId) {
      await axios.put(`http://localhost:3000/can-bo/${editId}`, form)
      setEditId(null)
    } else {
      await axios.post('http://localhost:3000/can-bo', form)
    }
  ...
  const handleDelete = async (id) => {
    ...
    await axios.delete(`http://localhost:3000/can-bo/${id}`)
    loadData()
  }
